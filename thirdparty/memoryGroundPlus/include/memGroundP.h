...
/**
 * @brief 排序函数指针类型（与标准库 qsort 签名兼容）
 * @param base  待排序数组基地址
 * @param nmemb 元素个数
 * @param size  每个元素大小
 * @param compar 比较函数
 */
typedef void (*mgp_sort_func_t)(void *base, size_t nmemb, size_t size,
                                 int (*compar)(const void *, const void *));

/**
 * @brief 二分查找函数指针类型（与标准库 bsearch 签名兼容）
 * @param key   查找键
 * @param base  有序数组基地址
 * @param nmemb 元素个数
 * @param size  每个元素大小
 ...
 */
typedef void *(*mgp_bsearch_func_t)(const void *key, const void *base,
...
/**
 * @brief 设置排序函数（必须在首次内存池操作前调用）
 * @param func 排序函数指针，传入 NULL 则禁用排序功能
 * @note 若未设置排序函数，涉及控制块排序的 API 将返回错误
 */
void mgp_set_sort_func(mgp_sort_func_t func);

/**
 * @brief 设置二分查找函数（必须在首次内存池操作前调用）
 * @param func 二分查找函数指针，传入 NULL 则禁用查找功能
 * @note 若未设置查找函数，涉及控制块查找的 API 将返回错误
 */
