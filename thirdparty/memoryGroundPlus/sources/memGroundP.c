 ...
 *          排序/查找函数通过回调注入，解除对 stdlib.h 的依赖。
 */
...
/* ==================== 可注入的排序/查找函数（解除 stdlib.h 依赖） ==================== */
static mgp_sort_func_t g_mgp_sort = NULL;
static mgp_bsearch_func_t g_mgp_bsearch = NULL;

void mgp_set_sort_func(mgp_sort_func_t func)
{
    g_mgp_sort = func;
}

void mgp_set_bsearch_func(mgp_bsearch_func_t func)
{
    g_mgp_bsearch = func;
}
...
const size_t mgp_canAllocMaxSize(mgp_t p)
{
    ...
    g_mgp_sort(pool->ctrlTable, pool->allocBlockCnt, sizeof(*pool->ctrlTable), mgp_ctrlBlockCompare);
#if MGP_MG_NUM
...
void mgp_free(mgp_t poolAddr, void *p)
{
    ...
    g_mgp_sort(pool->ctrlTable, pool->allocBlockCnt, sizeof(*pool->ctrlTable), mgp_ctrlBlockCompare);
    ctrl = g_mgp_bsearch(&vCtrl, pool->ctrlTable, pool->allocBlockCnt, sizeof(mgp_ctrl_t), mgp_ctrlBlockCompare);
    if (!ctrl)
    {
#if MGP_MEMORY_FREE_DEBUG
...
#if MGP_MG_NUM
    int ret = mgp_magicNumHCheck(ctrl);
    if (ret == MGP_ERR_CORRUPT)
    {
#if MGP_MEMORY_FREE_DEBUG
...
void* mgp_realloc(mgp_t poolAddr, void* src, const size_t bytes)
{
    ...
    if (!g_mgp_sort || !g_mgp_bsearch)
        goto _malloc; // 排序/查找函数未注入，直接尝试 malloc

    g_mgp_sort(pool->ctrlTable, pool->allocBlockCnt, sizeof(*pool->ctrlTable), mgp_ctrlBlockCompare);
    ctrl = g_mgp_bsearch(&vCtrl, pool->ctrlTable, pool->allocBlockCnt, sizeof(mgp_ctrl_t), mgp_ctrlBlockCompare); // 获取该内存块的控制块
    if (!ctrl) // 该内存块不在本内存池中
        goto _malloc;
 ...
 * @brief 控制块测试函数（调试用）
 * @param[in] pool 内存池指针
 * @note 使用确定性值替代 srand/rand，解除对 stdlib.h 的依赖
 */
static void mgp_ctrlBlockTest(mgp_pool_t *pool)
{
    ...
    // 使用确定性值填充测试数据（替代 srand/rand）
    static const mgp_ptr_t test_values[] = {0x0234, 0x089A, 0x0456, 0x0ABC,
                                             0x0678, 0x0DEF, 0x0012, 0x09FF};
    for (int i = 0; i < pool->allocBlockCnt; i++)
    {
#if MGP_MG_NUM
        pool->ctrlTable[i].pHead = (void *)test_values[i];
#else
        pool->ctrlTable[i].pBuff = (void *)test_values[i];
#endif
    }
